...
STRUCTURED_SYS = (
    ...
    "You will be given a single front-view image from the ego vehicle's dashboard camera. "
    "The image may include a gaze overlay showing where the driver is looking. "
    "Respond with a JSON object only — no extra explanation."
)
...
STRUCTURED_USER = (
    ...
    "  \"workzone_type\": <none | lane closure | worker activity | merging zone | mixed>,\n"
    "  \"traffic_condition\": <free flow | following vehicle | dense traffic | intersection | traffic light>,\n"
    "  \"primary_hazard\": <none | worker | cone | vehicle | traffic light | mixed>,\n"
    "  \"gaze_target\": <road center | worker | cone | vehicle | traffic light | irrelevant>,\n"
    "  \"attention_alignment\": <good | partial | poor>,\n"
    "  \"risk_level\": <low | medium | high>,\n"
    "  \"recommended_action\": <continue | slow down | stop | change lane>,\n"
    "  \"reasoning\": \"<short explanation based on scene + gaze + traffic>\"\n"
    "}"
)
